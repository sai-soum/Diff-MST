...
def knowledge_engineering_mix(
    ...
    if mix_console.num_track_control_params > 2:
        ...
        master_eq_lowshelf_params = torch.full((bs, 3),0)
        master_eq_lowshelf_params[:,:,1] = 100
        master_eq_lowshelf_params[:,:,2] = 1.0

        master_eq_bandpass0_params = torch.full((bs, 3),0)
        master_eq_bandpass0_params[:,:,1] = 500
        master_eq_bandpass0_params[:,:,2] = 1.0

        master_eq_bandpass1_params = torch.full((bs, 3),0)
        master_eq_bandpass1_params[:,:,1] = 3000
        master_eq_bandpass1_params[:,:,2] = 1.0

        master_eq_bandpass2_params = torch.full((bs, 3),0)
        master_eq_bandpass2_params[:,:,1] = 10000
        master_eq_bandpass2_params[:,:,2] = 1.0

        master_eq_bandpass3_params = torch.full((bs, 3),0)
        master_eq_bandpass3_params[:,:,1] = 13000
        master_eq_bandpass3_params[:,:,2] = 1.0

        master_eq_highshelf_params = torch.full((bs, 3),0)
        master_eq_highshelf_params[:,:,1] = 10000
        master_eq_highshelf_params[:,:,2] = 1.0


        master_comp_params = torch.full((bs, 6), -5)
        master_comp_params[:,:,1] = 1.0
        master_comp_params[:,:,2] = 10.0
        master_comp_params[:,:,3] = 10.0
        master_comp_params[:,:,4] = 3.0
        master_comp_params[:,:,5] = 0.0
    ...
    for j in range(bs):
        ...
        if use_fx_bus:
            fx_band_gain[j,0] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_0"][0], KE["fx_bus"]["reverb_gain"]["band_0"][1])
            fx_band_gain[j,1] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_1"][0], KE["fx_bus"]["reverb_gain"]["band_1"][1])
            fx_band_gain[j,2] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_2"][0], KE["fx_bus"]["reverb_gain"]["band_2"][1])
            fx_band_gain[j,3] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_3"][0], KE["fx_bus"]["reverb_gain"]["band_3"][1])
            fx_band_gain[j,4] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_4"][0], KE["fx_bus"]["reverb_gain"]["band_4"][1])
            fx_band_gain[j,5] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_5"][0], KE["fx_bus"]["reverb_gain"]["band_5"][1])
            fx_band_gain[j,6] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_6"][0], KE["fx_bus"]["reverb_gain"]["band_6"][1])
            fx_band_gain[j,7] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_7"][0], KE["fx_bus"]["reverb_gain"]["band_7"][1])
            fx_band_gain[j,8] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_8"][0], KE["fx_bus"]["reverb_gain"]["band_8"][1])
            fx_band_gain[j,9] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_9"][0], KE["fx_bus"]["reverb_gain"]["band_9"][1])
            fx_band_gain[j,10] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_10"][0], KE["fx_bus"]["reverb_gain"]["band_10"][1])
            fx_band_gain[j,11] = random.uniform(KE["fx_bus"]["reverb_gain"]["band_11"][0], KE["fx_bus"]["reverb_gain"]["band_11"][1])

            fx_band_decay[j,0] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_0"][0], KE["fx_bus"]["reverb_decay"]["band_0"][1])
            fx_band_decay[j,1] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_1"][0], KE["fx_bus"]["reverb_decay"]["band_1"][1])
            fx_band_decay[j,2] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_2"][0], KE["fx_bus"]["reverb_decay"]["band_2"][1])
            fx_band_decay[j,3] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_3"][0], KE["fx_bus"]["reverb_decay"]["band_3"][1])
            fx_band_decay[j,4] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_4"][0], KE["fx_bus"]["reverb_decay"]["band_4"][1])
            fx_band_decay[j,5] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_5"][0], KE["fx_bus"]["reverb_decay"]["band_5"][1])
            fx_band_decay[j,6] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_6"][0], KE["fx_bus"]["reverb_decay"]["band_6"][1])
            fx_band_decay[j,7] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_7"][0], KE["fx_bus"]["reverb_decay"]["band_7"][1])
            fx_band_decay[j,8] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_8"][0], KE["fx_bus"]["reverb_decay"]["band_8"][1])
            fx_band_decay[j,9] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_9"][0], KE["fx_bus"]["reverb_decay"]["band_9"][1])
            fx_band_decay[j,10] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_10"][0], KE["fx_bus"]["reverb_decay"]["band_10"][1])
            fx_band_decay[j,11] = random.uniform(KE["fx_bus"]["reverb_decay"]["band_11"][0], KE["fx_bus"]["reverb_decay"]["band_11"][1])

            fx_mix[j,0] = random.uniform(KE["fx_bus"]["mix"][0], KE["fx_bus"]["mix"][1])
            fx_send_db[j,0] = random.uniform(KE["fx_bus"]["send_db"][0], KE["fx_bus"]["send_db"][1])

        if use_master_bus:
            master_eq_lowshelf_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_lowshelf_gain"][0], KE["master_bus"]["eq"]["eq_lowshelf_gain"][1]
            )
            master_eq_lowshelf_params[j,1] = random.uniform(
                KE["master_bus"]["eq"]["eq_lowshelf_freq"][0], KE["master_bus"]["eq"]["eq_lowshelf_freq"][1]
            )
            master_eq_lowshelf_params[j,2] = random.uniform(
                KE["master_bus"]["eq"]["eq_lowshelf_q"][0], KE["master_bus"]["eq"]["eq_lowshelf_q"][1]
            )
            
            master_eq_bandpass0_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_band0_gain"][0], KE["master_bus"]["eq"]["eq_band0_gain"][1]
            )
            master_eq_bandpass0_params[j,1] =random.uniform(
                KE["master_bus"]["eq"]["eq_band0_freq"][0], KE["master_bus"]["eq"]["eq_band0_freq"][1]
            )
            master_eq_bandpass0_params[j,2] =random.uniform(
                KE["master_bus"]["eq"]["eq_band0_q"][0], KE["master_bus"]["eq"]["eq_band0_q"][1]
            )
            
            
            master_eq_bandpass1_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_band1_gain"][0], KE["master_bus"]["eq"]["eq_band1_gain"][1]
            )
            master_eq_bandpass1_params[j,1] =random.uniform(
                KE["master_bus"]["eq"]["eq_band1_freq"][0], KE["master_bus"]["eq"]["eq_band1_freq"][1]
            )
            master_eq_bandpass1_params[j,2] = random.uniform(
                KE["master_bus"]["eq"]["eq_band1_q"][0], KE["master_bus"]["eq"]["eq_band1_q"][1]
            )
            
            
            master_eq_bandpass2_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_band2_gain"][0], KE["master_bus"]["eq"]["eq_band2_gain"][1]
            )
            master_eq_bandpass2_params[j,1] = random.uniform(
                KE["master_bus"]["eq"]["eq_band2_freq"][0], KE["master_bus"]["eq"]["eq_band2_freq"][1]
            )
            master_eq_bandpass2_params[j,2] = random.uniform(
                KE["master_bus"]["eq"]["eq_band2_q"][0], KE["master_bus"]["eq"]["eq_band2_q"][1]
            )
        
            
            master_eq_bandpass3_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_band3_gain"][0], KE["master_bus"]["eq"]["eq_band3_gain"][1])
            master_eq_bandpass3_params[j,1] = random.uniform(
                KE["master_bus"]["eq"]["eq_band3_freq"][0], KE["master_bus"]["eq"]["eq_band3_freq"][1])
            master_eq_bandpass3_params[j,2] = random.uniform(
                KE["master_bus"]["eq"]["eq_band3_q"][0], KE["master_bus"]["eq"]["eq_band3_q"][1])
            
            
            master_eq_highshelf_params[j,0] = random.uniform(
                KE["master_bus"]["eq"]["eq_highshelf_gain"][0], KE["master_bus"]["eq"]["eq_highshelf_gain"][1])
            master_eq_highshelf_params[j,1] = random.uniform(
                KE["master_bus"]["eq"]["eq_highshelf_freq"][0], KE["master_bus"]["eq"]["eq_highshelf_freq"][1])
            master_eq_highshelf_params[j,2] = random.uniform(
                KE["master_bus"]["eq"]["eq_highshelf_q"][0], KE["master_bus"]["eq"]["eq_highshelf_q"][1])

            
            master_comp_params[j,0] = random.uniform(KE["master_bus"]["compressor"]["threshold_db"][0],
                    KE["master_bus"]["compressor"]["threshold_db"][1],
                )
            master_comp_params[j,1]  = random.uniform(
                    KE["master_bus"]["compressor"]["ratio"][0],
                    KE["master_bus"]["compressor"]["ratio"][1],
                )
            master_comp_params[j,2] = random.uniform(
                    KE["master_bus"]["compressor"]["attack_ms"][0],
                    KE["master_bus"]["compressor"]["attack_ms"][1],
                )
            master_comp_params[j,3] = random.uniform(
                    KE["master_bus"]["compressor"]["release_ms"][0],
                    KE["master_bus"]["compressor"]["release_ms"][1],
                )
            master_comp_params[j,4] = random.uniform(
                    KE["master_bus"]["compressor"]["knee_db"][0],
                    KE["master_bus"]["compressor"]["knee_db"][1],
                )
            master_comp_params[j,5] =random.uniform(
    ...
    # applying knowledge engineered compressor and EQ
        ...
        fx_bus_param_dict = {
            "reverberation": {
                "band0_gain": fx_band_gain[:,0],
                "band1_gain": fx_band_gain[:,1],
                "band2_gain": fx_band_gain[:,2],
                "band3_gain": fx_band_gain[:,3],
                "band4_gain": fx_band_gain[:,4],
                "band5_gain": fx_band_gain[:,5],
                "band6_gain": fx_band_gain[:,6],
                "band7_gain": fx_band_gain[:,7],
                "band8_gain": fx_band_gain[:,8],
                "band9_gain": fx_band_gain[:,9],
                "band10_gain": fx_band_gain[:,10],
                "band11_gain": fx_band_gain[:,11],
                "band0_decay": fx_band_decay[:,0],
                "band1_decay": fx_band_decay[:,1],
                "band2_decay": fx_band_decay[:,2],
                "band3_decay": fx_band_decay[:,3],
                "band4_decay": fx_band_decay[:,4],
                "band5_decay": fx_band_decay[:,5],
                "band6_decay": fx_band_decay[:,6],
                "band7_decay": fx_band_decay[:,7],
                "band8_decay": fx_band_decay[:,8],
                "band9_decay": fx_band_decay[:,9],
                "band10_decay": fx_band_decay[:,10],
                "band11_decay": fx_band_decay[:,11],
                "mix": fx_mix[:,0],
            }}
        master_bus_param_dict = {
            "parametric_eq": {
                "low_shelf_gain_db": master_eq_lowshelf_params[j,0],
                "low_shelf_cutoff_freq": master_eq_lowshelf_params[j,1],
                "low_shelf_q_factor": master_eq_lowshelf_params[j,2],
                "band0_gain_db": master_eq_bandpass0_params[j,0],
                "band0_cutoff_freq": master_eq_bandpass0_params[j,1],
                "band0_q_factor": master_eq_bandpass0_params[j,2],
                "band1_gain_db": master_eq_bandpass1_params[j,0],
                "band1_cutoff_freq": master_eq_bandpass1_params[j,1],
                "band1_q_factor": master_eq_bandpass1_params[j,2],
                "band2_gain_db": master_eq_bandpass2_params[j,0],
                "band2_cutoff_freq": master_eq_bandpass2_params[j,1],
                "band2_q_factor": master_eq_bandpass2_params[j,2],
                "band3_gain_db": master_eq_bandpass3_params[j,0],
                "band3_cutoff_freq": master_eq_bandpass3_params[j,1],
                "band3_q_factor": master_eq_bandpass3_params[j,2],
                "high_shelf_gain_db": master_eq_highshelf_params[j,0],
                "high_shelf_cutoff_freq": master_eq_highshelf_params[j,1],
                "high_shelf_q_factor": master_eq_highshelf_params[j,2]
            },
            # release and attack time must be the same
            "compressor": {
                "threshold_db": master_comp_params[j,0],
                "ratio": master_comp_params[j,1],
                "attack_ms": master_comp_params[j,2],
                "release_ms": master_comp_params[j,3],
                "knee_db": master_comp_params[j,4],
                "makeup_gain_db": master_comp_params[j,5],
            },
        }
