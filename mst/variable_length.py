...
class AttentionPool(torch.nn.Module):
    def __init__(self, embed_dim: int):
        super().__init__()
        self.lin1 = nn.Linear(embed_dim, embed_dim // 2)
        self.lin2 = nn.Linear(embed_dim // 2, 1)

    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        e = self.lin2(torch.tanh(self.lin1(x))).squeeze(-1)  # (B, N)
        if mask is not None:
            e = e.masked_fill(mask, -1e9)
        alpha = torch.softmax(e, dim=-1).unsqueeze(-1)  # (B, N, 1)
...
class VariableLengthEncoder(torch.nn.Module):
    def __init__(
        ...
        chunk_seconds: float = 5.0,
        hop_seconds: Optional[float] = None,
        pool_mode: int = 0,  # 0 = mean, 1 = max, 2 = attention
        embed_dim: int = 512
    ):
        super().__init__()
        ...
        self.sr = sr
        self.chunk_len = int(chunk_seconds * sr)
        self.hop_len = int((hop_seconds if hop_seconds is not None else chunk_seconds) * sr)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        chunks = torch.stack([x[..., s : s + self.chunk_len] for s in starts], dim=0)
        chunks = chunks.reshape(-1, C, self.chunk_len)
        embeds = self.base_encoder(chunks)  # (total_chunks, D)

        num_chunks = starts.shape[0]
        embeds = embeds.view(B, num_chunks, -1)  # (B, N, D)

        if self.pool_mode == 0:
            return embeds.mean(dim=1)
        elif self.pool_mode == 1:
            return embeds.max(dim=1).values
        else:
            return self.attn_pool(embeds)
