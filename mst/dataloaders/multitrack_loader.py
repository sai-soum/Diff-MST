...
class multitrack_dataset(torch.utils.data.Dataset):
    ...
    def reload_buffer(self):
        ...
        for dir in pbar:
            pbar.set_description("Loading dataset buffer")
            track_filepaths = glob.glob(os.path.join(dir, "*.wav"))
            if len(track_filepaths) < self.min_tracks and len(track_filepaths) < self.max_tracks:
                continue
            ...
            track_metadata =[]
            stereo_info = []
            offset = np.random.randint(0.25 * num_frames , num_frames - self.buffer_frames - 1)

            for track_filepath in track_filepaths:
                stereo = False
                track, _ = torchaudio.load(track_filepath, frame_offset=offset, num_frames=self.buffer_frames)
                ...
                track_lufs_db = self.meter.integrated_loudness(track.permute(1, 0).numpy())
                ...
                instrument = self.song_dirs[dir][os.path.basename(track_filepath)]
